...
export default function UploadSingleFile(props: any) {
  ...
  const upload = useCallback(async () => {
    ...
    if (response.data.data) {
      const existStoredWhitelists = JSON.parse(localStorage.getItem('storedWhitelists')!);
      ...
      if (existStoredWhitelists) {
        ...
        localStorage.setItem('storedWhitelists', JSON.stringify(storedWhitelists));
      }
      setLoader(true);
      setTimeout(() => {
        setLoader(false);
    ...
    <Box sx={{width: '100%'}}>
      ...
      >
        <TextField required={true} value={whitelistName} onChange={handleChangeData} fullWidth
                   placeholder="Collection Name" sx={{color:"grey"}}
                   variant="outlined"/>
